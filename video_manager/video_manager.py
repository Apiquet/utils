...
def main():
    ...
    parser.add_argument(
        "-i", "--input_videos", required=True, type=str, help="Video path separated by a comma."
    )
    parser.add_argument(
        "-k",
        "--skip",
    ...
    parser.add_argument("--fps", required=False, type=int, default=30, help="New fps to set")
    parser.add_argument(
        "-r",
        "--resize_fact",
        required=False,
    ...
    parser.add_argument("-o", "--output_path", required=False, type=str, help="Output path.")
    ...
    if args.output_path is None:
        args.output_path = os.path.dirname(videos_path[0]) + '/output_video.mp4'
    ...
    out = cv2.VideoWriter(args.output_path, 0x7634706D, args.fps, (width, height))
