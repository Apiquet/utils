...
def main():
    ...
    parser.add_argument("-i", "--input_pdf", required=True, type=str, help="Path to the pdf file.")
    parser.add_argument("-o", "--output_pdf", required=True, type=str, help="Path to the pdf file.")
    parser.add_argument(
        "-w",
        "--wishto",
        required=True,
        choices=['remove', 'keep', 'compress', 'changeorder', 'overlay', 'merge'],
        type=str,
        help="Option wanted.",
    )
    parser.add_argument(
        "-g", "--gs_path", required=False, default=None, type=str, help="path/to/gswin64c.exe"
    )
    parser.add_argument(
        "-c",
        "--power_compression",
    ...
    parser.add_argument(
        "-p", "--pages", required=False, type=str, help="List of pages concerned: 1,2,5,11"
    )
    parser.add_argument(
        "-m",
        "--merge_overlay",
    ...
    if args.wishto == "compress":
        assert args.gs_path is not None, "You should specify GhostScript path to compress pdf file"
        compress_pdf(args.input_pdf, args.output_pdf, args.power_compression, args.gs_path)
        ...
        overlay_pdf_files(args.input_pdf, args.output_pdf, args.merge_overlay, args.pages)
